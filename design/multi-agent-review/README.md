...
| 02 / 04 | **Not a departure — a correction (2026-07-17).** Configure run and Results are two distinct routes (`/multi-agent-review?pr=<id>` and `/multi-agent-review/results?pr=<id>`), and starting a run from *either* entry point navigates to Results | The mocks always showed this (04 has no PR picker and its own header row + breadcrumb `… › #482`), and the spec's flow diagram always routed both entry points to `M[Results page]` — but no AC encoded the transition, so the first implementation rendered results inside Configure's step-2 card and the PR page never navigated at all. Recorded here because this table's silence previously read as "considered and rejected"; it was neither. See SPEC-05 AC-5b / AC-18 |
