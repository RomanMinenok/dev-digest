...
export const agentRuns = pgTable(
  'agent_runs',
  {
    ...
    /** Membership in a multi-agent run (nullable — single-agent/CI runs carry no membership). */
    ...
    /** Estimated cost in USD at run completion (null = unknown model / failed run). */
    costUsd: doublePrecision('cost_usd'),
  },
  (table) => ({
    multiAgentRunIdIdx: index('agent_runs_multi_agent_run_id_idx').on(table.multiAgentRunId),
  }),
);
