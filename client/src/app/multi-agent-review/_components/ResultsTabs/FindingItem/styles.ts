...
export const s = {
  ...
  metaRow: { display: "flex", alignItems: "center", gap: 12, marginTop: 5 } satisfies CSSProperties,
  location: { fontSize: 12.5, color: "var(--text-secondary)" } satisfies CSSProperties,
  ...
  actions: { display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" } satisfies CSSProperties,
} as const;

export function findingIconColor(severity: string): string {
  return SEV_COLOR[severity] ?? SEV_COLOR_FALLBACK;
}
